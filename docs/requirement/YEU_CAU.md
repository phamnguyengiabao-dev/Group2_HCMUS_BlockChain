...
- Tập validator cố định cho mỗi lần chạy, thỏa `n = 3f + 1`; hỗ trợ ít nhất 8 node.
- Giao dịch, header và vote đều được ký số; khóa công khai và `chain_id` cố định cho mỗi lần chạy.
- Thực thi trạng thái key-value xác định (deterministic) và lưu cam kết trạng thái (state commitment) vào mỗi header.
- Mạng P2P mô phỏng với: độ trễ có thể cấu hình, giới hạn băng thông/tốc độ, nhân bản thông điệp, sắp xếp lại, rớt, và chặn peer tạm thời.
...
- Merkle Patricia Trie đầy đủ — hash chuẩn của toàn bộ trạng thái đã sắp xếp là đủ.
...
|---|---|
| **An toàn (Safety)** | Không có hai block finalize khác nhau tại cùng một chiều cao giữa các node đúng đắn, kể cả trước khi có synchrony. |
| **Hoạt động (Liveness)** | Sau điểm ổn định và khi chọn được proposer đúng đắn, một chiều cao sau đó phải được finalize. |
| **Hợp lệ (Validity)** | Parent không hợp lệ, proposer sai, chữ ký/domain sai, giao dịch sai, state hash sai, hoặc trường vote sai đều bị từ chối và ghi log. |
| **Kiểm đếm vote** | Vote trùng lặp không tăng trọng số; chỉ một vote hợp lệ mỗi validator cho mỗi `(height, round, phase)`. |
| **Tất định (Determinism)** | Cùng kịch bản/cấu hình/seed tạo ra log byte-identical và state hash cuối giống nhau trên hai lần chạy. |
...
- Baseline dùng Ed25519 và canonical JSON-like bytes theo đặc tả giao thức. Nếu ngôn ngữ lập trình không đảm bảo serializer ổn định, hãy triển khai bộ mã hóa nhị phân tường minh thay thế.
- Chọn dung lượng block mặc định, lịch timeout, và chính sách lưu giữ dữ liệu trong `config/default.json`; mỗi giá trị này phải được đưa vào fingerprint kịch bản được ghi vào log.
